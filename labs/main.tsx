...
const createPresets = (): Record<PresetName, GlassParams & { radius: number }> => {
  ...
  return {
    ...
    subtle: { ...DEFAULT_PARAMS, refraction: 30, thickness: 40, gloss: 30, softness: 5, saturation: 30, dispersion: 20, displacementResolution: 100, displacementMinResolution: 20, displacementSmoothing: 0, enableOptimization: 1, refreshInterval: 1, radius: 16 },
    bold: { ...DEFAULT_PARAMS, refraction: 80, thickness: 70, gloss: 70, softness: 15, saturation: 60, dispersion: 50, displacementResolution: 100, displacementMinResolution: 20, displacementSmoothing: 0, enableOptimization: 1, refreshInterval: 1, radius: 32 },
    frosted: { ...DEFAULT_PARAMS, refraction: 40, thickness: 50, gloss: 40, softness: 50, saturation: 20, dispersion: 60, displacementResolution: 50, displacementMinResolution: 15, displacementSmoothing: 20, enableOptimization: 1, refreshInterval: 1, radius: 20 },
    crystal: { ...DEFAULT_PARAMS, refraction: 70, thickness: 60, gloss: 80, softness: 0, saturation: 70, dispersion: 40, displacementResolution: 100, displacementMinResolution: 25, displacementSmoothing: 0, enableOptimization: 1, refreshInterval: 1, radius: 28 },
    ios: { ...DEFAULT_PARAMS, refraction: 45, thickness: 55, gloss: 55, softness: 20, saturation: 50, dispersion: 35, displacementResolution: 75, displacementMinResolution: 20, displacementSmoothing: 10, enableOptimization: 1, refreshInterval: 1, radius: 22 },
  };
};
...
const RENDER_STEP_COLORS: Record<RenderStep, string> = {
  getBounds: '#60a5fa', getStyle: '#a78bfa', prediction: '#f472b6',
  displacementMap: '#fb923c', specularMap: '#facc15', svgUpdate: '#4ade80', morph: '#22d3d8',
};

const RENDER_STEP_LABELS: Record<RenderStep, string> = {
  getBounds: 'Bounds', getStyle: 'Style', prediction: 'Predict',
  displacementMap: 'Disp Map', specularMap: 'Spec Map', svgUpdate: 'SVG', morph: 'Morph',
};

const RENDER_STEPS: RenderStep[] = ['getBounds', 'getStyle', 'prediction', 'displacementMap', 'specularMap', 'svgUpdate', 'morph'];
...
interface ExtendedFrameTiming {
  frameId: number; timestamp: number; deltaMs: number; totalMs: number;
  steps: Record<RenderStep, number>; hasRender: boolean;
}
...
function PerformanceGraph({ enabled }: { enabled: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      const frameTiming: ExtendedFrameTiming = {
        frameId: frameIdRef.current++, timestamp: Date.now(), deltaMs,
        totalMs: renderData?.totalMs ?? 0,
        steps: renderData?.steps ?? { getBounds: 0, getStyle: 0, prediction: 0, displacementMap: 0, specularMap: 0, svgUpdate: 0, morph: 0 },
  ...
  useEffect(() => {
    if (frames.length === 0) { setAverages(null); setFrameDropCount(0); return; }
    const sums: Record<RenderStep, number> = { getBounds: 0, getStyle: 0, prediction: 0, displacementMap: 0, specularMap: 0, svgUpdate: 0, morph: 0 };
    let drops = 0, renderFrameCount = 0;
    for (const frame of frames) {
      if (frame.deltaMs > FRAME_DROP_THRESHOLD_MS) drops++;
      if (frame.hasRender) { renderFrameCount++; for (const step of RENDER_STEPS) sums[step] += frame.steps[step]; }
    }
    if (renderFrameCount > 0) { for (const step of RENDER_STEPS) sums[step] /= renderFrameCount; }
    setAverages(sums); setFrameDropCount(drops);
  }, [frames]);
  ...
  useEffect(() => {
    ...
    canvas.style.width = `${totalWidth}px`; ctx.scale(dpr, dpr);
    ctx.fillStyle = 'rgba(0, 0, 0, 0.2)'; ctx.fillRect(0, 0, totalWidth, height);
    ...
    frames.forEach((frame, i) => {
      ...
      if (frame.deltaMs > FRAME_DROP_THRESHOLD_MS) {
        ctx.fillStyle = 'rgba(255, 59, 48, 0.4)'; ctx.fillRect(x, 0, FRAME_BAR_WIDTH, height);
      }
      ...
      if (frame.hasRender && frame.totalMs > 0) {
        ...
        for (const step of RENDER_STEPS) {
          const stepHeight = (frame.steps[step] / maxDelta) * chartHeight;
          if (stepHeight > 0.3) { ctx.fillStyle = RENDER_STEP_COLORS[step]; ctx.fillRect(x, y - stepHeight, FRAME_BAR_WIDTH - FRAME_BAR_GAP, stepHeight); y -= stepHeight; }
        }
      } else {
        ctx.fillStyle = 'rgba(80, 80, 80, 0.5)'; ctx.fillRect(x, baseY - deltaHeight, FRAME_BAR_WIDTH - FRAME_BAR_GAP, deltaHeight);
      }
    });
    ...
    const drawLine = (ms: number, color: string, label: string) => {
      ...
      if (lineY > 5 && lineY < height - 10) {
        ...
        ctx.beginPath(); ctx.moveTo(0, lineY); ctx.lineTo(totalWidth, lineY); ctx.stroke(); ctx.setLineDash([]);
        ctx.fillStyle = color; ctx.font = '9px -apple-system, sans-serif'; ctx.fillText(label, 3, lineY - 2);
      }
    };
    drawLine(16.67, 'rgba(52, 199, 89, 0.7)', '60fps');
    drawLine(33.33, 'rgba(255, 204, 0, 0.7)', '30fps');
...
function ToggleButtons<T extends string>({ label, description, value, options, onChange }: {
...
function InteractiveElement({ data, selected, onSelect, onDelete, onUpdate, glassParams, tintColor, tintOpacity, previewAreaRef }: {
  ...
  const dragStateRef = useRef<{ type: 'drag' | 'resize' | 'rotate' | null; startX: number; startY: number; startData: { x: number; y: number; w: number; h: number; r: number }; resizeDir?: string; }>({ type: null, startX: 0, startY: 0, startData: { x: 0, y: 0, w: 0, h: 0, r: 0 } });
...
function ParameterLab() {
  ...
  const applyPreset = (name: PresetName) => {
    ...
    for (const paramName of PARAMETER_NAMES) { schemaParams[paramName] = preset[paramName] as never; }
    setParams(schemaParams); setRadius(preset.radius); setActivePreset(name);
  };

  const codeOutput = useMemo(() => {
    const lines = ['.glass-panel {'];
    for (const name of PARAMETER_NAMES) { lines.push(`  ${getCSSPropertyName(name)}: ${formatCSSValue(name, params[name])};`); }
    lines.push(`  border-radius: ${radius}px;`); lines.push('}');
    return lines.join('\n');
  }, [params, radius]);
          ...
          {PARAMETER_NAMES.map((name) => {
            ...
            if (def.type === 'number') {
              ...
              if (numDef.transform === 'boolean') {
                return <ToggleButtons key={name} label={formatParameterName(name)} description={numDef.description}
      ...
      <main class="preview-area" ref={previewAreaRef}>
        ...
        {elements.map((el) => (
          <InteractiveElement key={el.id}
            ...
            selected={selectedId === el.id} onSelect={() => setSelectedId(el.id)}
