/**
 * Full-pass displacement computation (Single-Pass)
 *
 * Computes displacement for ALL pixels in a single pass.
 * Benchmark results show this is 1.2-1.3x faster than 2-pass quadrant approach.
 *
 * Uses point-symmetry properties to compute displacement vectors directly
 * without requiring texture sampling or intermediate render targets.
 *
 * RGB encoding:
...
fn main(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    let fullWidth = uniforms.u_resolution.x;
    let fullHeight = uniforms.u_resolution.y;
    ...
    // Pixel coordinates (0.5 offset for pixel center)
    let px = fragCoord.x - 0.5;
    let py = fragCoord.y - 0.5;

    // Distance from center (absolute value for symmetry)
    let dx = abs(px - halfW + 0.5);
    let dy = abs(py - halfH + 0.5);

    // Sign for displacement direction based on quadrant
    let signX = select(-1.0, 1.0, px >= halfW);
    let signY = select(-1.0, 1.0, py >= halfH);
